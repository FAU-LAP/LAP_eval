...
def coupling(lamb_list,d,n_Au_list,n_SiC_list,E_0=1,y=-0.05,x=-0.01):
    
    r_Au_list=ref_E(1,n_Au_list)
    r_Au_list = 0.83*r_Au_list+y*1j ## with correction for imperfect scattering and refelction phase
    n_SiC_list = ri.n_from_string('SiC').get_n(lamb_list)
    k=2*const.pi/lamb_list
    r_SiC_list = ref_E(1,n_SiC_list)+x*1j
    t_SiC_list = trans_E(n_SiC_list,1)+x*1j
    return n_SiC_list*np.abs(E_0*t_SiC_list*(1-1/r_SiC_list/(1-(1/((np.exp(2j*k*d)*r_Au_list*r_SiC_list))))))**2

def thermal_radiation_mirror(lamb,d,A,T,n_Au,n_SiC):

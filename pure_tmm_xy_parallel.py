...
def use_parallel(filepath):
    ...
    if (525 < float(search_result[1]) < 535) and (-15 < float(search_result[2]) < -7):
        ...
        return search_result[1], search_result[2], result_vac, result_mem


parallel_result = Parallel(n_jobs=2)(delayed(use_parallel)(file_path) for file_path in get_regex_matches(folder_path))
